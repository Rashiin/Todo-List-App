    ...
    <div className="p-4 fixed left-0 bg-[#ffcc99] z-50 right-0 ">
      <header className="flex justify-between  items-center">
        ...
        <div className="flex space-x-4 items-center md:space-x-6">
          ...
          <button className="button1 py-2 px-3 md:hidden"
            onClick={() => {
            setOpenAddEditTask( state => !state)
          }}>
+       
            ...
            className="cursor-pointer h-6" />
          {
            ...
            setOpenEditModal={setOpenEditModal}  type='Boards' />
          }
        </div>
      </header>
      {openDropdown && <HeaderDropdown setBoardModalOpen={setBoardModalOpen} setOpenDropdown={setOpenDropdown} />}
      {
        boardModalOpen && <AddEditBoard type={boadType} setBoardModalOpen={setBoardModalOpen} />
      }
      {
